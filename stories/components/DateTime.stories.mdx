...
import { DatePicker, DateRangePicker, TimePicker, TimeRangePicker, DateTimePicker, DateTimeRangePicker } from '../../src';
      ...
      return (
        <div>
          <TimeRangePicker
            ...
            onChange={(value) => { setSelectedDates(value) }}
            shortcuts={[
              { label: '早餐时间', value: () => [set(new Date(0), { hours: 8 }), set(new Date(0), { hours: 8, minutes: 30 })] },
              { label: '下午茶时间', value: () => [set(new Date(0), { hours: 16 }), set(new Date(0), { hours: 16, minutes: 30 })] },
            ]}
          />
        </div>
      )
      ...
      return (
        <div>
          <DateTimePicker
            ...
            value={selectedDate}
            onChange={(value) => { console.log('onchange', value); setSelectedDate(value) }}
            shortcuts={[
              { label: '今天', value: () => startOfToday() },
              { label: '明天', value: () => startOfTomorrow() },
              { label: '下周一', value: () => setDay(add(new Date(), { weeks: 1 }), 1) },
...
### 日期时间区间选择器 DateTimeRangePicker

<Canvas>
  <Story name="DateTimeRangePicker">
    {() => {
      const [selectedDates, setSelectedDates] = useState(null)
      return (
        <div>
          <DateTimeRangePicker
            startPlaceholder={"请选择开始日期和时间"}
            endPlaceholder={"请选择结束日期和时间"}
            value={selectedDates}
            onChange={(value) => { setSelectedDates(value) }}
            shortcuts={[
              { label: '本周', value: () => [startOfWeek(new Date()), endOfWeek(new Date())] },
...
<ArgsTable components={{ DatePicker, DateRangePicker, TimePicker, DateTimePicker, DateTimeRangePicker }} />